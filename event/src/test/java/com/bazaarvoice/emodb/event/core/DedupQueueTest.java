...
public class DedupQueueTest {
    ...
    public void testPollSkipsEmptyChannels() {
        ...
        verify(readerDao).readNewer(eq("read"), any());
        verify(readerDao).readNewer(eq("write"), any());
    ...
    public void testPeekChecksAllChannels() {
        ...
        verify(readerDao).readAll(eq("read"), any(), isNull(), eq(true));
        verify(readerDao).readNewer(eq("write"), any());
        verifyNoMoreInteractions(readerDao);
    ...
    private SortedQueueFactory getPersistentSortedQueueFactory() {
        ...
        Mockito.when(factory.create(anyString(), any(QueueDAO.class))).thenAnswer(new Answer() {
