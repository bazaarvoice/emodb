...
public class DedupQueueClientFactory extends AbstractDedupQueueClientFactory {
    ...
    public static DedupQueueClientFactory forCluster(String clusterName) {
        HttpClientConfiguration httpClientConfiguration = new HttpClientConfiguration();
        httpClientConfiguration.setKeepAlive(Duration.seconds(1));
        return new DedupQueueClientFactory(clusterName, createDefaultJerseyClient(httpClientConfiguration));
    }
    ...
    public static DedupQueueClientFactory forClusterAndHttpConfiguration(String clusterName, HttpClientConfiguration configuration) {
        return new DedupQueueClientFactory(clusterName, createDefaultJerseyClient(configuration));
    }
