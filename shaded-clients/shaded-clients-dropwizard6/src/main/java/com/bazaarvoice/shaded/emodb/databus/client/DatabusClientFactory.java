package com.bazaarvoice.shaded.emodb.databus.client;
...
public class DatabusClientFactory extends AbstractDatabusClientFactory {
    ...
    public static DatabusClientFactory forCluster(String clusterName) {
        HttpClientConfiguration httpClientConfiguration = new HttpClientConfiguration();
        httpClientConfiguration.setKeepAlive(Duration.seconds(1));
        return new DatabusClientFactory(clusterName, createDefaultJerseyClient(httpClientConfiguration));
    }
    ...
    public static DatabusClientFactory forClusterAndHttpConfiguration(String clusterName, HttpClientConfiguration configuration) {
        return new DatabusClientFactory(clusterName, createDefaultJerseyClient(configuration));
    }

    private DatabusClientFactory(String clusterName, Client jerseyClient) {
