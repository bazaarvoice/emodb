...
public class DefaultRateLimitedLogFactoryTest {
    ...
    public void testSingleError() {
        ...
        RateLimitedLog rateLimitedLog =
                new DefaultRateLimitedLogFactory(executor, Duration.days(1))
                        .from(log);
        Mockito.verify(executor).scheduleWithFixedDelay(any(), eq(1L), eq(1L), eq(TimeUnit.MINUTES));
        Throwable t = new Throwable();

        rateLimitedLog.error(t, "Test error: {}", "first!");

        Mockito.verify(log).error("Test error: first!", t);
        Mockito.verify(executor).schedule(any(Runnable.class), eq(1L), eq(TimeUnit.DAYS));
    ...
    public void testMultipleErrors() {
        ...
        Mockito.verify(executor).schedule(captor1.capture(), eq(30L), eq(TimeUnit.SECONDS));
        Mockito.verifyNoMoreInteractions(log, executor);
        ...
        Mockito.verify(executor).schedule(captor2.capture(), eq(30L), eq(TimeUnit.SECONDS));
        Mockito.verifyNoMoreInteractions(log, executor);
