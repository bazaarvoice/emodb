 ...
 */
public class JaxRSEmoResource implements EmoResource {

    private WebTarget _target;
    private Invocation.Builder _builder;
    private MediaType _type;

    JaxRSEmoResource(WebTarget resource) {
        _target = checkNotNull(resource, "target");
    }

    @Override
    public EmoResource queryParam(String key, String value) {
        checkState(_target != null, "Invalid state to add a query param");
    ...
    public EmoResource path(String path) {
        checkState(_target != null, "Invalid state to add a path");
    ...
    private EmoResponse toEmoResponse(Response response) {
        return new JaxRSEmoResponse(response);
    }
    ...
    private MediaType type() {
        checkNotNull(_type, "You must set the media type for the request");
        return _type;
    }
}
