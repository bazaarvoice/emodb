 ...
 * interpreted correctly by the Jersey HTTP resource {@link DedupQueueResource1}.
 */
public class DedupQueueJerseyTest extends ResourceTest {
    private static final String APIKEY_QUEUE = "queue-key";
    private static final String APIKEY_UNAUTHORIZED = "unauthorized-key";

    private final PartitionContextValidator<AuthDedupQueueService> _pcxtv =
            OstrichAccessors.newPartitionContextTest(AuthDedupQueueService.class, DedupQueueClient.class);
    private final DedupQueueService _server = mock(DedupQueueService.class);
    ...
    public ResourceTestRule _resourceTestRule = setupResourceTestRule(
            Collections.<Object>singletonList(new DedupQueueResource1(_server, DedupQueueServiceAuthenticator.proxied(_proxy), _mockStatsDClient)),
            ImmutableMap.of(
    ...
    private DedupQueueService queueClient() {
        return DedupQueueServiceAuthenticator.proxied(new DedupQueueClient(URI.create("/dedupq/1"), new JerseyEmoClient(_resourceTestRule.client())))
                .usingCredentials(APIKEY_QUEUE);
    }

    private DedupQueueService queueClient(boolean partitioned) {
        return DedupQueueServiceAuthenticator.proxied(new DedupQueueClient(URI.create("/dedupq/1"), partitioned, new JerseyEmoClient(_resourceTestRule.client())))
                .usingCredentials(APIKEY_QUEUE);
    }

    private DedupQueueService unauthorizedQueueClient(boolean partitioned) {
        return DedupQueueServiceAuthenticator.proxied(new DedupQueueClient(URI.create("/dedupq/1"), partitioned, new JerseyEmoClient(_resourceTestRule.client())))
    ...
    public void testGetMessageCount() {
        long expected = 123L;
        when(_proxy.getMessageCount(APIKEY_QUEUE, "queue-name")).thenReturn(expected);

        long actual = queueClient().getMessageCount("queue-name");

        assertEquals(actual, expected);
        verify(_proxy).getMessageCount(APIKEY_QUEUE, "queue-name");
        verifyNoMoreInteractions(_proxy);
    }

    @Test
    public void testGetMessageCountPartitioned() {
        long expected = 123L;
        when(_server.getMessageCount("queue-name")).thenReturn(expected);

        long actual = queueClient(true).getMessageCount("queue-name");
    ...
    public void testGetMessageCountUpTo() {
        long expected = 123L;
        when(_proxy.getMessageCountUpTo(APIKEY_QUEUE, "queue-name", 10000L)).thenReturn(expected);

        long actual = queueClient().getMessageCountUpTo("queue-name", 10000L);

        assertEquals(actual, expected);
        verify(_proxy).getMessageCountUpTo(APIKEY_QUEUE, "queue-name", 10000L);
        verifyNoMoreInteractions(_proxy);
    }

    @Test
    public void testGetMessageCountUpToPartitioned() {
        long expected = 123L;
        when(_server.getMessageCountUpTo("queue-name", 10000L)).thenReturn(expected);

        long actual = queueClient(true).getMessageCountUpTo("queue-name", 10000L);
    ...
    public void testPeek() {
        ...
        when(_proxy.peek(APIKEY_QUEUE, "queue-name", 123)).thenReturn(expected);

        List<Message> actual = queueClient().peek("queue-name", 123);

        assertEquals(actual, expected);
        verify(_proxy).peek(APIKEY_QUEUE, "queue-name", 123);
        verifyNoMoreInteractions(_proxy);
    }

    @Test
    public void testPeekPartitioned() {
        List<Message> expected = ImmutableList.of(new Message("id-1", "payload-1"), new Message("id-2", "payload-2"));
        when(_server.peek("queue-name", 123)).thenReturn(expected);

        List<Message> actual = queueClient(true).peek("queue-name", 123);
